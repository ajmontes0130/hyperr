...
export default async function(req) {
  try {
    ...
    if (!creator_id) return Response.json({ error: 'creator_id is required' }, { status: 400 });

    const reviews = await base44.entities.Review.filter({ creator_id }, '-created_date', 200);
